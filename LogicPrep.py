...
class LogicPreps:
    ...
    def sort_list_by_float_ele(self, data_list, ele_idx, up_down_flag=True):
        result_list = []
        for tmp_arr in sorted(data_list, key=lambda tmp_arr: float(tmp_arr[ele_idx].replace('"', '').replace("'", "")),
                              reverse=up_down_flag):
